...
public class HotelService {
	...
	public synchronized List<Hotel> findAll(String nameFilter, String addressFilter) {
		...
		for (Hotel hotel : hotels.values()) {
			try {
			    boolean namePasses = (nameFilter == null || nameFilter.isEmpty()
                        || hotel.getName().toLowerCase().contains(nameFilter.toLowerCase()));
			    boolean addressPasses = (addressFilter == null || addressFilter.isEmpty()
                        || hotel.getAddress().toLowerCase().contains(addressFilter.toLowerCase()));
