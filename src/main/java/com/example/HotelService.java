...
public class HotelService {
    ...
    public synchronized List<Hotel> findAll(String nameFilter, String addressFilter) {
        ...
        for (Hotel hotel : hotels.values()) {
            try {
                boolean namePasses = passesFilter(nameFilter, hotel.getName());
                boolean addressPasses = passesFilter(addressFilter, hotel.getAddress());
                boolean passesFilter = (namePasses && addressPasses);
    ...
    public void delete(Iterable<Hotel> iterable){
        iterable.forEach(hotel -> hotels.remove(hotel.getId()));
    }
    ...
    public void ensureTestData() {
        if (findAll().isEmpty()) {
            ...
            for (String hotel : hotelData) {
                ...
                h.setCategory(categoryService.getCategory(r.nextInt(categoryService.getCount())));
                h.setOperatesFrom(System.currentTimeMillis() - 90 * 60 * 60 * 1000);
                save(h);
            }
        }
    }
